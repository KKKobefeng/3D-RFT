...
s1 = scatter3(points_include(:,1), points_include(:,2), points_include(:,3), 'filled', 'MarkerEdgeColor', 'none', 'CData', pressures(:,1), 'SizeData', 250*max_pressure/max(abs(pressures(:,2)))*abs(pressures(:,1)));
colormap(jet);
...
hold on;
trimesh(TRG_visual, 'LineWidth', 0.1, 'EdgeColor', '#888888', 'FaceAlpha', 0);
s2 = scatter3(points_include(:,1), points_include(:,2), points_include(:,3), 'filled', 'MarkerEdgeColor', 'none', 'CData', pressures(:,2), 'SizeData', 250*max_pressure/max(abs(pressures(:,2)))*abs(pressures(:,2)));
colormap(jet);
clim([-max_pressure max_pressure]);
setPlotProperties(x_range, y_range, z_range);
...
s3 = scatter3(points_include(:,1), points_include(:,2), points_include(:,3), 'filled', 'MarkerEdgeColor', 'none', 'CData', pressures(:,3), 'SizeData', 250*max_pressure/max(abs(pressures(:,2)))*abs(pressures(:,3)));
colormap(jet);
clim([-max_pressure max_pressure]);
setPlotProperties(x_range, y_range, z_range);
